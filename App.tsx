          ...
          {!output && !isLoading && !error && (
             <div className="flex items-center justify-center h-full bg-slate-100 dark:bg-slate-800/50 rounded-xl border-2 border-dashed border-slate-300 dark:border-slate-700">
                <div className="text-center p-8">
                    ...
                    <h3 className="mt-2 text-sm font-semibold text-slate-900 dark:text-slate-100">{t('app.outputPlaceholderTitle')}</h3>
                    <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{t('app.outputPlaceholderDescription')}</p>
                </div>
            </div>
          )}
        </div>
